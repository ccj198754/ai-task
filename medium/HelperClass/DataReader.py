...
class DataReader(object):
    ...
    def ReadData(self):
        ...
        if train_file.exists():
            classofflower = {b'Iris-setosa': 0, b'Iris-versicolor': 1, b'Iris-virginica': 2}
            data = np.loadtxt(self.train_file_name, delimiter=',', skiprows=1, converters={4: lambda m: classofflower[m]})
            self.XTrainRaw = data[:, :4]
            self.YTrainRaw = data[:, 4:]
